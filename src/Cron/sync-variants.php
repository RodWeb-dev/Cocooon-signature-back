...
$materialsMap = [];
$rows = $db->query('SELECT id, name FROM materials')->fetchAll(PDO::FETCH_ASSOC);
foreach ($rows as $row) {
...
foreach ($templates as $template) {
    ...
    $variants    = $odoo->getVariantsForTemplate((int) $template['id']);

    foreach ($variants as $variant) {
        [$dimension, $materialName] = parseDisplayName($variant['display_name']);
