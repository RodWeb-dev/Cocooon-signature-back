...
class CartModel
{
    ...
    /** Soft-deletes a cart by setting its status to cancelled (owner guards against cross-cart edits). */
    public static function deleteCart(int $cartId, string $userId): void
    {
        $sql = "UPDATE carts SET status = 'cancelled'
    ...
    /** Sets the quantity of a cart item (owner guards against cross-cart edits). */
    public static function updateItemQuantity(int $itemId, int $quantity, string $userId): void
    {
        $sql = "UPDATE cart_items AS ci
                JOIN carts AS c ON c.id = ci.cart_id
                SET ci.quantity = :quantity
                WHERE ci.id = :id AND c.owner = :owner";
    ...
    /** Removes an item from the cart (owner guards against cross-cart deletes). */
    public static function deleteItem(int $itemId, string $userId): void
    {
        $sql = "DELETE ci FROM cart_items AS ci
                JOIN carts AS c ON c.id = ci.cart_id
                WHERE ci.id = :id AND c.owner = :owner";
    ...
    /** Returns the user's pending cart with its items and primary product images, or null. */
    public static function findPendingCart(string $userId): ?array
    {
        $sql = "SELECT * FROM carts WHERE owner = :owner AND status = 'pending'";
